...
{
    public partial class MaterialRequireUpdate : Form
    {
        private TreeNode tn;
        public string ProjectId = string.Empty, ecprojectid = "",meo_manual="";
        ...
        public void listviewTitleBind()
        {
            ...
            if (MEOID != "0")
            {
                ...
                if (meolist != null)
                {
                    ProjectId = meolist.PROJECT_ID;
                    //tn_meono.Text = meolist;
                    meo_manual = meolist.REQUIRE_NO;
                    tb_manual.Text = meolist.REQUIRE_NO;
                    ...
                    tb_area.Text = meolist.SYSTEM_ID;
                    tb_fx.Text = meolist.PARTTYPE_ID;
                    tb_discipline.Text = meolist.DISCIPLINEID;
                    tb_type.Text = meolist.ERP_PARTTYPE;
                    tb_manual.Text = meolist.REQUIRE_NO;
                    Str_Area = meolist.SYSTEM_ID;
                    Str_FX = meolist.PARTTYPE_ID;
                    Str_Discipline = meolist.ERP_DISCIPLINE;
                    ActivityName = meolist.ERP_PARTTYPE;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
           ...
           string Site = cmb_site.SelectedValue.ToString();
            ...
            if (meolist.DISCIPLINEID != cmb_discipline.SelectedValue.ToString())
            {
            ...
            if (MEOID != "0")
            {
                ...
                for (int i = 0; i < rowcou; i++)
                {
                    ...
                    string preQty = dgv1.Rows[i].Cells["申请数量"].Value.ToString().Trim();
                    string meonoerp = "";
                    string remark = dgv1.Rows[i].Cells["remark"].Value.ToString();
                    if (dgv1.Rows[i].Cells["MEO_ERP"].Value != null)
                    {
        ...
        private void dgv1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (MEOSTATE == "1")
            {
                if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
                {
                    ...
                    dgv1.Rows[e.RowIndex].Cells["需求原因"].Value = "1001";
                }
                if (dgv1.RowCount == 0)
                {
                    dgv1.Rows.Add();
                    dgv1.Rows[0].Cells["MEO_ID"].Value = MEOID;
                    dgv1.Rows[0].Cells["需求原因"].Value = "1001";
                }
                SetRowNo();
            }
        }
        ...
        {
            if (dgv1.RowCount != 0 && dgv1.SelectedRows.Count!=0)
            {
        ...
        private void btn_new_Click(object sender, EventArgs e)
        {
            if (meolist.STATE == "1")
            {
                MessageBox.Show("此单据尚未通过审批!不能导出!");
                return;
            }
            InitializeWorkbook();
            ...
            for (int i = 0; i < dgv1.RowCount; i++)
            {
                ...
                sheet1.GetRow(i + 5).GetCell(2).SetCellValue(ProjectId);
                sheet1.GetRow(i + 5).GetCell(3).SetCellValue(Str_Area);
                sheet1.GetRow(i + 5).GetCell(4).SetCellValue(Str_FX);
                sheet1.GetRow(i + 5).GetCell(5).SetCellValue(Str_Discipline);
                sheet1.GetRow(i + 5).GetCell(6).SetCellValue(ActivityName);
                sheet1.GetRow(i + 5).GetCell(7).SetCellValue(partno);
                sheet1.GetRow(i + 5).GetCell(8).SetCellValue(partname);
            ...
            {
                WriteToFile();
                MessageBox.Show(meo_manual + ".xls" + "导出成功!","提示消息");
            }
            catch (Exception ex)
            {
        ...
        private  void WriteToFile()
        {
            ...
            string exportpath = "MEO导出/" + meo_manual+".xls";
        ...
        static void InitializeWorkbook()
        {
            ...
            FileStream file = new FileStream(@"Template/项目MEO明细表.xls", FileMode.Open, FileAccess.Read);
